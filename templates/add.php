    ...
    <form class="form form--add-lot container <?= empty($errors) ? '' : 'form--invalid' ?>" action="/add.php" method="post" enctype="multipart/form-data">
        ...
        <div class="form__container-two">
            <div class="form__item <?= isset($errors['lot-name']) ? 'form__item--invalid' : '' ?>">
                ...
                <input id="lot-name" type="text" name="lot-name" placeholder="Введите наименование лота" value="<?= $formInputs['lot-name'] ?? '' ?>">
            ...
            <div class="form__item <?= isset($errors['category']) ? 'form__item--invalid' : '' ?>">
                ...
                <select id="category" name="category">
                    ...
                    <?php foreach ($cats as $category) : ?>
                        <option value="<?= $category['id']; ?>" <?= isset($formInputs['category']) && $formInputs['category'] === $category['id'] ? 'selected' : '' ?>>
                            <?= $category['name'] ?? ''; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
        ...
        <div class="form__item form__item--wide <?= isset($errors['description']) ? 'form__item--invalid' : '' ?>">
            ...
            <textarea id="message" name="description" placeholder="Напишите описание лота"><?= $formInputs['description'] ?? '' ?></textarea>
        ...
        <div class="form__container-three">
            <div class="form__item form__item--small <?= isset($errors['lot-price']) ? 'form__item--invalid' : '' ?>">
                ...
                <input id="lot-rate" type="text" name="lot-price" placeholder="0" value="<?= $formInputs['lot-price'] ?? '' ?>">
                <span class="form__error"><?= $errors['lot-price'] ?? ''; ?></span>
            </div>
            <div class="form__item form__item--small <?= isset($errors['lot-step']) ? 'form__item--invalid' : '' ?>">
                <label for="lot-step">Шаг ставки <sup>*</sup></label>
                <input id="lot-step" type="text" name="lot-step" placeholder="0" value="<?= $formInputs['lot-step'] ?? '' ?>">
