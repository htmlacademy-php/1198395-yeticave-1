        ...
        <div class="lot-item__content">
            <div class="lot-item__left">
                <div class="lot-item__image">
                    <img
                        src="<?= isset($lot['img_url']) ? htmlspecialchars($lot['img_url']) : ''; ?>"
                        width="730"
                        height="548"
                        alt="<?= isset($lot['name']) ? htmlspecialchars($lot['name']) : ''; ?>"
                    />
                </div>
                <p class="lot-item__category">Категория: <span><?= isset($lot['category']) ? htmlspecialchars(
                            $lot['category']
                        ) : ''; ?></span></p>
                <p class="lot-item__description"><?= isset($lot['description']) ? htmlspecialchars(
                        $lot['description']
                    ) : ''; ?></p>
            </div>
            <div class="lot-item__right">
                <div class="lot-item__state">
                    ...
                    <?php
                    if ($hours === '00' && $minutes === '00' || isset($lot['winner_id'])) : ?>
                        <p>Торги окончены.</p>
                    <?php
                    else : ?>
                    ...
                    if ($showBids): ?>
                        <form
                            ...
                            action="/lot.php?id=<?= isset($lot['id']) ? htmlspecialchars($lot['id']) : ''; ?>"
                            method="post"
                            autocomplete="off"
                        >
                            <p class="lot-item__form-item form__item <?= empty($errors)
                                ? '' : 'form__item--invalid'; ?>">
                                <label for="cost">Ваша ставка</label>
                                <input id="cost" type="text" name="cost"
                                       placeholder="<?= $minBid; ?>"
                                       value="<?= isset($formInputs['cost']) ? htmlspecialchars(
                                           $formInputs['cost']
                                       ) : ''; ?>"/>
                                <?php
                                if (!empty($errors)): ?>
                ...
                <div class="history">
                    ...
                    <table class="history__list">
                        ...
                        foreach ($bids as $bid) : ?>
                            <?php
                            if (isset($bid['user_name'], $bid['amount'], $bid['created_at'])) : ?>
